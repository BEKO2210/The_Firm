...
- [x] **B.2** — Mission-Control-Layout (statisch): Home-Seite neu — Zone 1 Status-Zeile (5 North-Star-KPI-Karten, Schema Label→Wert→Δ→Zeitraum) + Zone 2 JETZT/NÄCHSTES/LETZTES, server-side aus `state.json` + `audit.log` + Quotes. Komponenten `KpiCard`, `MissionColumns`. Lighthouse 100/100/100/100 + axe 0 Violations bestätigt.
